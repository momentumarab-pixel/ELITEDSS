...
// Delt i to grupper: modellvurderinger vs. kampstatistikk
const RANG_MODELL = [
  {
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0 ml-3">
          <div className="flex items-center rounded-lg border border-white/10 overflow-hidden" style={{ background: "rgba(255,255,255,0.04)" }}>
            <input type="text" value={inp}
              ...
              className="w-10 text-center text-xs font-bold focus:outline-none py-1 tabular-nums"
              style={{ background: "transparent", color: INDIGO.c, caretColor: INDIGO.c }} />
...
// ══════════════════════════════════════════════════════════════════════════
// ── Gjenbrukbar rangering-blokk ───────────────────────────────────────────
function RangKategoriBlokk({ kat, isOpen, sorted, top, onToggle }: {
  kat: { id: string; label: string; enhet: string; decimaler: number; forklaring: string; color: string; multiply100?: boolean }
  isOpen: boolean; sorted: Player[]; top: number; onToggle: () => void
}) {
  return (
    <div className="rounded-2xl border border-white/6 overflow-hidden transition-all"
        ...
        <div className="flex items-center gap-3">
          {sorted[0] && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-white/45">{sorted[0].player_name.split(" ").pop()}</span>
      ...
      {isOpen && (
        <div className="border-t border-white/5">
          <div className="px-5 py-3 border-b border-white/4" style={{ background: "rgba(129,140,248,0.03)" }}>
            <p className="text-[10px] text-white/38 leading-relaxed">{kat.forklaring}</p>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {sorted.map((p, i) => {
              const raw = (p as any)[kat.id] ?? 0
              const val = kat.multiply100 ? raw * 100 : raw
              const barPct = Math.min((val / top) * 100, 100)
                ...
                <div key={p.player_name} className="flex items-center gap-3 px-5 py-2.5 hover:bg-white/2 transition-colors">
                  ...
                  <div className="w-36 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[9px] text-white/25 truncate max-w-[80px]">{p.team_name}</span>
  ...
  return (
    <div className="min-h-screen bg-bg0" style={{ fontFamily: "'DM Sans',sans-serif" }}>
      <style>{`
        ...
        input:-webkit-autofill,input:-webkit-autofill:hover,input:-webkit-autofill:focus{-webkit-text-fill-color:#ffffff!important;-webkit-box-shadow:0 0 0 1000px rgba(20,20,35,0.98) inset!important;caret-color:#ffffff!important}
      ...
      <div className="max-w-[1400px] mx-auto px-5 py-5">
        ...
        {tab === "rangering" && (
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                ...
                <input value={rangSearch} onChange={e => setRangSearch(e.target.value)}
                  ...
                  className="border border-white/10 rounded-xl pl-8 pr-3 py-2 text-xs focus:outline-none focus:border-indigo-500/40 w-44"
            ...
            <p className="text-[10px] text-white/25 mb-4">Trykk på en kategori for å se topp 10 — med forklaring av hva statistikken betyr.</p>

            {/* ── GRUPPE 1: Modellvurderinger ── */}
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-4 rounded-full" style={{ background: INDIGO.c }} />
                <span className="text-xs font-bold text-white">Modellvurderinger</span>
                <span className="text-[10px] text-white/25">— beregnet av systemet fra all tilgjengelig data</span>
              </div>
              <div className="space-y-1.5">
                {RANG_MODELL.map(kat => {
                  ...
                  const sorted = [...filtered].sort((a, b) => ((b as any)[kat.id] ?? 0) - ((a as any)[kat.id] ?? 0)).slice(0, 10)
                  const top = sorted[0] ? (kat.multiply100 ? ((sorted[0] as any)[kat.id] || 0) * 100 : ((sorted[0] as any)[kat.id] || 0)) : 1
                  return (
                    <RangKategoriBlokk key={kat.id} kat={kat} isOpen={isOpen} sorted={sorted} top={top}
                      onToggle={() => setOpenRangCat(isOpen ? null : kat.id)} />
                  )
                })}
              </div>
            </div>

            {/* ── GRUPPE 2: Kampstatistikk ── */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-4 rounded-full" style={{ background: AMBER.c }} />
                <span className="text-xs font-bold text-white">Kampstatistikk</span>
                <span className="text-[10px] text-white/25">— faktiske tall fra kampene, justert per 90 minutter</span>
              </div>
              <div className="space-y-1.5">
                {RANG_KAMP.map(kat => {
                  const isOpen = openRangCat === kat.id
                  ...
                  const sorted = [...filtered].sort((a, b) => ((b as any)[kat.id] ?? 0) - ((a as any)[kat.id] ?? 0)).slice(0, 10)
                  const top = sorted[0] ? ((sorted[0] as any)[kat.id] || 0) : 1
                  return (
                    <RangKategoriBlokk key={kat.id} kat={kat} isOpen={isOpen} sorted={sorted} top={top}
                      onToggle={() => setOpenRangCat(isOpen ? null : kat.id)} />
                  )
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div>
          ...
          <input value={role.name} onChange={e => setRole(r => ({ ...r, name: e.target.value }))}
            ...
            className="w-full border border-white/10 rounded-xl px-3 py-2.5 text-sm font-medium focus:outline-none focus:border-indigo-500/50"
            style={{ background: "rgba(255,255,255,0.06)", color: "#ffffff" }} />
        </div>
        <div>
